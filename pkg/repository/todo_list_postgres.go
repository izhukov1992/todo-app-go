...
func (r *TodoListPostgres) Update(userId, listId int, list todo.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id=ul.list_id AND ul.user_id=$%d AND ul.list_id=$%d", todoListsTable, querySet, usersListsTable, argId, argId+1)
